...
from utils import strxor, hexdec, hexenc
    

class Stribog:
    def __init__(self, data = b"", hash_size: int = 512) -> None:
        ...
        self.__A = [unpack(">Q", hexdec(s))[0] for s in (
        ...
        self.__C = [hexdec("".join(s))[::-1] for s in (
            (
    ...
    def update(self, data) -> None:
        """Обновляет хешируемые данные"""
        self.__data = data

    def hash(self):
        """Возвращает хеш для указанного self.__data в hex формате"""
        return hexenc(self.__digest())

    def __digest(self):
        """Вовзращает хеш для указанного self.__data"""
        hsh = 64 * (b'\x01' if self.__hash_size == 256 else b'\x00')
        chk = 64 * b'\x00'
        counter = 0
        data = self.__data
        for i in range(0, len(data) // 64 * 64, 64):
            block = data[i:i + 64]
            hsh = self.__g(counter, hsh, block)
            chk = self.__add512mod(chk, block)
            counter += 512

        # Padding
        padblock_size = len(data) * 8 - counter
        data += b'\x01'
        padlen = 64 - len(data) % 64
        if padlen != 64:
            data += b'\x00' * padlen

        hsh = self.__g(counter, hsh, data[-64:])
        counter += padblock_size
        chk = self.__add512mod(chk, data[-64:])
        hsh = self.__g(0, hsh, pack("<Q", counter) + 56 * b'\x00')
        hsh = self.__g(0, hsh, chk)
        if self.__hash_size == 256:
            return hsh[32:]
        return hsh

    def __add512mod(self, a: bytearray, b: bytearray) -> bytearray:
        """Складывает два 512 битных числа"""
        temp = 0
        res = bytearray(64)
        for i in range(64):
            temp = a[i] + b[i] + (temp >> 8)
    ...
    def __g(self, n, hsh, msg):
        """Функция сжатия"""
        res = self.__E(self.__LPS(strxor(hsh[:8], pack("<Q", n)) + hsh[8:]), msg)
        return strxor(strxor(res, hsh), msg)

    def __E(self, k, msg):
        for i in range(12):
            msg = self.__LPS(strxor(k, msg))
            k = self.__LPS(strxor(k, self.__C[i]))
        return strxor(k, msg)

    def __LPS(self, data):
       return self.__L(self.__PS(data))

    def __PS(self, data):
        """Преобразует входные данные data в массив байтов, используя таблицы перестановок"""
        return bytearray(self.__Pi[data[self.__Tau[i]]] for i in range(64))
